...
export default async function FAQPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: isZh
        ? '是的，完全免费。无需账号、无需订阅、无使用限制。在线工具永久免费。'
        : 'Yes, completely free. No account, no subscription, no usage limits. The online tool is free forever.',
    },
    {
    ...
    },
    {
      ...
      a: isZh
        ? '在线工具可在任何现代浏览器中使用，无需安装。'
        : 'The online tool works in any modern browser without installation.',
    },
    {
      q: isZh ? '它支持 Google AI Studio 吗？' : 'Does it work on Google AI Studio?',
      a: isZh
        ? '支持。使用在线工具可以去除 Google AI Studio 图片的水印。'
        : 'Yes. Use the online tool to remove watermarks from Google AI Studio images.',
    },
    {
      q: isZh ? '处理后水印仍然存在，为什么？' : 'The watermark still appears after processing. Why?',
      a: isZh
        ? '部分图片可能包含多层水印。请确保上传的是原始图片，并使用在线工具进行处理。'
        : 'Some images may have multiple watermark layers. Make sure you upload the original image and use the online tool for processing.',
    },
    {
      q: isZh ? 'CleanMark 多久更新一次？' : 'How often is CleanMark updated?',
      a: isZh
        ? '我们定期发布更新，以保持与 Gemini 和 AI Studio 变更的兼容性、提升检测精度并添加新功能。'
        : 'We release updates regularly to maintain compatibility with Gemini and AI Studio changes, improve detection accuracy, and add new features.',
    },
    {
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="py-12 px-4 bg-blue-600 text-center">
        ...
        <Link href="/gemini-watermark-remover" className="inline-flex items-center gap-2 bg-white text-blue-600 hover:bg-gray-50 px-8 py-3.5 rounded-xl font-semibold transition shadow-lg">
